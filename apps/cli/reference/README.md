...
The production Web runner needs built package and frontend artifacts (`pnpm run build`). It serves `http://127.0.0.1:3080` by default. `--host` also accepts an explicit internal IPv4 literal such as `100.64.0.10`; the CLI intentionally does not support `--host 0.0.0.0` yet and exits with a usage error. `--trusted-host` adds named authorities accepted by the `/api` browser-trust fence.
