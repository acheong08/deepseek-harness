...
生产 Web 运行器需要已构建的包和前端产物（`pnpm run build`）。默认服务地址是 `http://127.0.0.1:3080`。`--host` 还接受显式内部 IPv4 字面量，例如 `100.64.0.10`；CLI 目前仍有意不支持 `--host 0.0.0.0`，并会以用法错误退出。`--trusted-host` 可添加 `/api` 浏览器信任围栏接受的具名 authority。
