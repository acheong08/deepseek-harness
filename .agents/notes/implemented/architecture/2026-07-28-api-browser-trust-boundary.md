...
- Non-loopback deployments must have their serving authorities trusted or requests are refused. The Web runtime derives the machine's LAN IP literals into the connection row for `0.0.0.0` binds (port-less entries — an IP-literal Host cannot be a rebound name, and the bound port may be OS-assigned), trusts an explicit internal bind literal directly, and offers `dsh web --trusted-host` for named authorities; compositions the CLI does not boot declare `trustedHosts` themselves. Non-browser automation rides the same fence: loopback, a derived LAN IP, an explicit internal bind literal, or a declared authority passes; an undeclared DNS alias is refused.
