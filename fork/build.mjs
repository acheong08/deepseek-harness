 ...
 *
 * The fork's only functional change is relaxing the `dsh web` bind host to
 * accept private/CGNAT IPv4 literals (see fork/README.md). Publishing that
 * without mirroring the whole ~200-package monorepo means republishing just
 * four packages under a new scope, with every other dependency pinned to the
 * published upstream `@deepseek-ai/*` versions.
 *
 * This script reproduces that from a committed ref: it stages a detached
 * worktree (the working tree is never modified), renames the four fork
...
// The four packages this fork republishes. `suffix` is the unscoped npm name.
const FORK_PACKAGES = [
...
// Vendored upstream packages the four fork packages depend on, at the ranges
...
function applyReplacements(text, scope, replacements) {
  let out = text
...
function main() {
  ...
  try {
    ...
    // 2. Rewrite source self-references and the web-app bundle patch mounts.
    ...
    // 4. Pack the four packages with publication manifests.
    ...
    for (const suffix of ['dsh-host-webserver', 'dsh-app-boot', 'dsh-web-app', 'dsh']) {
